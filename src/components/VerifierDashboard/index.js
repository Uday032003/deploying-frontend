...
class VerifierDashboard extends Component {
  state = {
    verifierAppliedLoansSectionDetails: [],
  ...
  onSuuccessCall = (data) => {
    this.setState({
      verifierAppliedLoansSectionDetails: data,
  ...
  getLoansDetails = async () => {
    const url =
      "https://deploying-backend-14-b2t2.onrender.com/verifierAppliedLoans/";
    const options = {
      method: "GET",
    };
    try {
  ...
  renderSuccessView = () => {
    const { verifierAppliedLoansSectionDetails } = this.state;
    return (
